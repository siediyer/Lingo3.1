...
"""
6G 显卡稳定版说明：
1. 将 TTS 并发固定为 1，避免显存和接口压力叠加。
2. 保留原有 warmup 和时间线处理逻辑，尽量减少行为变化。
3. 这个文件只负责 TTS 生成与时间轴处理，不控制 WhisperX 的 batch_size。
   WhisperX 的 batch_size 需要在调用 transcribe 的文件里单独改。
"""

TEMP_FILE_TEMPLATE = f"{_AUDIO_TMP_DIR}/{{}}_temp.wav"
OUTPUT_FILE_TEMPLATE = f"{_AUDIO_SEGS_DIR}/{{}}.wav"

# 保守一些，前几条先顺序跑，便于尽早暴露问题
WARMUP_SIZE = 3
    ...
    for attempt in range(max_retries):
        try:
            ...
            if output_duration >= expected_duration * 1.02 and input_duration < 3 and diff <= 0.1:
                ...
                trimmed_audio = audio[:(expected_duration * 1000)]
...
def process_row(row: pd.Series, tasks_df: pd.DataFrame) -> Tuple[int, float]:
    """Generate TTS for one row and calculate actual duration."""
    number = row['number']
...
def generate_tts_audio(tasks_df: pd.DataFrame) -> pd.DataFrame:
    """Generate TTS audio with conservative settings for 6G GPU."""
    tasks_df['real_dur'] = 0
    rprint("[bold green]🎯 Starting TTS audio generation (6G safe mode)...[/bold green]")
    rprint(f"[cyan]ℹ️ TTS workers forced to {SAFE_MAX_WORKERS} for stability[/cyan]")
        ...
        if row['cut_off'] == 1:
            ...
            if cur_time > chunk_end_time:
                ...
                if time_diff <= 0.6:
                    ...
                    trimmed_audio = audio[:(new_duration * 1000)]
                    ...
                    raise Exception(
                        f"Chunk {chunk_start} to {index} exceeds the chunk end time {chunk_end_time:.2f} seconds "
                        f"with current time {cur_time:.2f} seconds"
                    )
            chunk_start = index + 1
...
def gen_audio() -> None:
    ...
    rprint("[bold magenta]🚀 Starting audio generation process (6G safe mode)...[/bold magenta]")
