...
def siliconflow_fish_tts(text, save_path, mode='preset', voice_id=None, ref_audio=None, ref_text=None, check_duration=False):
    ...
    if mode == 'preset':
        ...
        safe_ref_audio, size_mb, duration_sec = ensure_reference_under_limit(ref_audio, max_mb=4.3, initial_max_sec=8.0)
        with open(safe_ref_audio, 'rb') as f:
            audio_base64 = base64.b64encode(f.read()).decode('utf-8')
        rprint(f"[blue]Using dynamic reference audio: {safe_ref_audio} | {duration_sec:.2f}s | {size_mb:.2f}MB[/blue]")
        payload = {
            'model': MODEL_NAME,
            'response_format': 'wav',
            'stream': False,
...
def create_custom_voice(audio_path, text, custom_name=None):
    ...
    safe_audio_path, size_mb, duration_sec = ensure_reference_under_limit(audio_path, max_mb=4.3, initial_max_sec=8.0)
    audio_base64 = f"data:audio/wav;base64,{base64.b64encode(open(safe_audio_path, 'rb').read()).decode('utf-8')}"
    rprint(f'[yellow]✅ Successfully encoded audio file | {duration_sec:.2f}s | {size_mb:.2f}MB')
    ...
    rprint('[yellow]🚀 Sending request to create voice...')
...
def get_ref_audio(task_df):
    ...
    return safe_audio, combined_text
    ...
    if MODE == 'preset':
        ...
        if log_name != custom_name:
            ...
            if ref_audio is None or ref_text is None:
                rprint('[red]Failed to get reference audio and text, falling back to preset mode')
                return siliconflow_fish_tts(text, save_as, mode='preset')
